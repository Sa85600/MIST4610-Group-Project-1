...
-  Aarav Patel
-  Saianagha Attili [@sa85600](https://github.com/Sa85600/MIST4610-Group-Project-1)
...
Each patient has one medical record and each medical record can only belong to one patient so the tables have a one to one relationship. Medical records maintain the diagnoses, prescribed medications, and diseases related to each patient. The MedicalRecords entity has a many to many relationship with the Diseases table. A medical record may contain many diseases because a patient might have many diseases, and many patients might have the same disease so it might be on multiple records. There is a purely associative table between the two with a composite primary key called MedicalRecordsDiseases. Similarly, Diseases and Medications have a many to many relationship, as do Medications and Patients. Diseases can have many medications to treat them, and medications can be used for many diseases. The medications are associated with the diseases through the Treatments table. Treatments and Medications are linked to diseases, allowing doctors to prescribe the necessary treatments to manage patient conditions. Patients can be taking many medications and medications can be prescribed to many patients. The Prescriptions table holds this information. Prescriptions track the medications prescribed to each patient, including dosage and frequency.

There is a one to many relationship between Insurance Policies and Patients, as many patients can be on one policy or plan. There is also a one to many relationship between Insurance Providers and Insurance Policies because insurance providers have many plans for customers to choose from. Insurance Providers and Policies help in managing billing and financial transactions related to hospital services.
